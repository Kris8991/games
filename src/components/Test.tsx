...
useEffect(() => {
    if (flippedCards.length === 2) {
      ...
      if (cardsState[first].value === cardsState[second].value) {
        console.log(cardsState[first].value, cardsState[second].value);
        setMathedCards([...mathedCards, first, second]);
        setFlippedCards([]);
        setIsDisabled(false);
      } else {
        setTimeout(() => {
  ...
  }, [cardsState, flippedCards, mathedCards]);
